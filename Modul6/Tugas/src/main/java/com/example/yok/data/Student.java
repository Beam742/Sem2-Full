...
//        }
    ...
    public void choiceBook(String bookId, int duration) {
        for (Book book : Student.getStudentBook()) {
            if (book.getBookId().equals(bookId) && book.getStock() > 0) {
                Book borrowedBook = new Book(book.getBookId(), book.getTitle(), book.getAuthor(), 1, book.getCategory());
    ...
    public static void setStudentBook() {
        studentBook.clear();
        for (Book book : Admin.getBookList()) {
            Book copiedBook = new Book(book.getBookId(), book.getTitle(), book.getAuthor(), book.getStock(), book.getCategory());
